...
import { gearSet, jewel, PieceInSet, RawPiece, stats } from "../types";
import { pieceTypesClient, pieceTypesDb, rareness, tableNames } from "../enums";
import { gearSetPiecesCount } from "../consts";
import { convertPieceTypesClientsToPieceTypesDb, convertRawPieceToPieceInSet } from "./utils.functions";
...
export const getPieceByIdAndRareness = async (db: SQLiteDatabase, pieceId: number | null, pieceRareness: rareness | null, jewelsByPieceId: number | null, temperness_level: number): Promise<RawPiece | undefined> => {   
    try{
...
export const getAllPiecesByTypeAndRareness = async (db: SQLiteDatabase, type: pieceTypesClient | pieceTypesDb, piecesRareness: rareness | undefined): Promise<RawPiece[]> => {
    try{

        let dbPieceType = convertPieceTypesClientsToPieceTypesDb(type)

        if(piecesRareness === undefined) {
            piecesRareness = rareness.common
        }

        const piecesArray: RawPiece[] = []
        const sqlQuery: string = `SELECT piece_id FROM ${tableNames.pieces} WHERE  (type = "${dbPieceType}") ORDER BY piece_id`
        ...
        for(const piece of pieces) { //never use arr.foreach(async () => {}) use this because of the issues
            for (let i = 0; i < piece.rows.length; i++) {
                const currentPiece: RawPiece | undefined = await getPieceByIdAndRareness(
                    db, piece.rows.item(i).piece_id, piecesRareness, null, 0
                )
    ...
    try{
        ...
        const allTempernessSets: [ResultSet] = await db.executeSql(sqlQueryGetTempernessLevels)
            ...
            for (let i = 0; i < allGearSets[0].rows.length; i++) {
                ...
                mainHand: convertRawPieceToPieceInSet(
                    await getPieceByIdAndRareness(db, allGearSets[0].rows.item(i).mainHand,
                    ...
                    pieceTypesClient.mainHand,
                ),

                helmet: convertRawPieceToPieceInSet(
                    await getPieceByIdAndRareness(db, allGearSets[0].rows.item(i).helmet,
                    ...
                    pieceTypesClient.helmet,
                ),

                plate: convertRawPieceToPieceInSet(
                    await getPieceByIdAndRareness(db, allGearSets[0].rows.item(i).plate,
                    ...
                    pieceTypesClient.plate,
                ),

                boots: convertRawPieceToPieceInSet(
                    await getPieceByIdAndRareness(db, allGearSets[0].rows.item(i).boots,
                    ...
                    pieceTypesClient.boots,
                ),

                secondHand: convertRawPieceToPieceInSet(
                    await getPieceByIdAndRareness(db, allGearSets[0].rows.item(i).secondHand,
                    ...
                    pieceTypesClient.secondHand,
                ),

                accessory1: convertRawPieceToPieceInSet(
                    await getPieceByIdAndRareness(db, allGearSets[0].rows.item(i).accessory1,
                    ...
                    pieceTypesClient.accessory1,
                ),

                accessory2: convertRawPieceToPieceInSet(
                    await getPieceByIdAndRareness(db, allGearSets[0].rows.item(i).accessory2,
                    ...
                    pieceTypesClient.accessory2,
                ),

                accessory3: convertRawPieceToPieceInSet(
                    await getPieceByIdAndRareness(db, allGearSets[0].rows.item(i).accessory3,
...
export const updateGearSet = async (db: SQLiteDatabase, gearSet: gearSet, title: string | null): Promise<boolean> => {
    ...
    gearSet.mainHand ? updatePieceInGearSetByIdAndType(db, gearSet.id, gearSet.mainHand) : null
    gearSet.helmet ? updatePieceInGearSetByIdAndType(db, gearSet.id, gearSet.helmet) : null
    gearSet.plate ? updatePieceInGearSetByIdAndType(db, gearSet.id, gearSet.plate) : null
    gearSet.boots ? updatePieceInGearSetByIdAndType(db, gearSet.id, gearSet.boots) : null
    gearSet.secondHand ? updatePieceInGearSetByIdAndType(db, gearSet.id, gearSet.secondHand) : null
    gearSet.accessory1 ? updatePieceInGearSetByIdAndType(db, gearSet.id, gearSet.accessory1) : null
    gearSet.accessory2 ? updatePieceInGearSetByIdAndType(db, gearSet.id, gearSet.accessory2) : null
    gearSet.accessory3 ? updatePieceInGearSetByIdAndType(db, gearSet.id, gearSet.accessory3) : null
...
export const updatePieceInGearSetByIdAndType = async (db: SQLiteDatabase, gearSetId: number, newPiece: PieceInSet): Promise<boolean> => {
    try{
